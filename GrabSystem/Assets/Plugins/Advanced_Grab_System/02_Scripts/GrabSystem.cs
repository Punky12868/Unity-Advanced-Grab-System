...
public class GrabSystem : MonoBehaviour
{
    ...
    [SerializeField] private bool massAffectsMovement = true;
    ...
    private void GrabObject()
    {
        if (Physics.Raycast(cameraPos.position, cameraPos.forward, out RaycastHit hit, grabRange, GrabMask))
        {
            ...
            if (hitRigidbody != null && massAffectsMovement) GetComponent<FirstPersonCharacterController>().SetDraggingState(true, hitRigidbody.mass);
    ...
    {
        target = null;
        if (massAffectsMovement) GetComponent<FirstPersonCharacterController>().SetDraggingState(false);
